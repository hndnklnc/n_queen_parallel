...
Genetic::Genetic(int size, int num_thread){
    SIZE = size;
    comm_size = num_thread;
...
int* repairChoromosome(vector<int> sameElements, Choromosome parent, Choromosome child, int index1, int index2, vector<int> exElements){
	...
	return child.geneCode;
}



vector<int*> Crossover(Choromosome parent1, Choromosome parent2){
	...
	children.push_back(repairChoromosome(difP2C1Elements, parent1, child1, randpos1, randpos2, difP1C2Elements));
	children.push_back(repairChoromosome(difP1C2Elements, parent2, child2, randpos1, randpos2, difP2C1Elements));
	delete pieceCode1;
	delete pieceCode2;
...
void makeNewInitialization(){
	...
	cout<<"tour time:"<< (tour_end - tour_begin)<<" sec"<<endl;
