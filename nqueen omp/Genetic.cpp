...
Genetic::Genetic(int size, int num_thread, int granularity){
    SIZE = size;
...
vector<int> findDifference(int* piece1, int* piece2, int pieceSize){
	...
	bool isSame = false;
	if(pieceSize > forgra){
		#pragma omp parallel for num_threads(thread_count) private(isSame)
...
float findFitness(int* code){
	...
	int counter;
	if(gra > diagSize){
		#pragma omp parallel for num_threads(thread_count) reduction(+:sum) private(counter)
